...
window.renderAdminactivity = function (container) {
  ...
  if (tab === 'custom-leads') {
    container.innerHTML =
      ...
      '" />' +
      '</div>' +
      '<p class="admin-muted ca-custom-lede">One row per lead — photo, client, status, car details, cart, and last activity.</p>' +
      '<div id="caHost">' +
      (U.skeletonTable ? U.skeletonTable(6) : '<div class="admin-loading">Loading…</div>') +
      '</div>';
    ...
    function loadCustomLeads() {
      ...
      fetchJson('/api/customer-activity/custom-leads?' + apiQuery()).then(function (res) {
          ...
          rows
            .map(function (lead) {
              ...
              var photoHtml = photoUrl
                ? '<a class="ca-custom-row-thumb" href="' +
                  esc(photoUrl) +
                  ...
                  '" loading="lazy" />' +
                  (moreCount ? '<span class="ca-custom-row-more">+' + moreCount + '</span>' : '') +
                  '</a>'
                : '<span class="ca-custom-row-thumb ca-custom-row-thumb--empty">—</span>';
              ...
              return (
                '<tr class="ca-custom-row">' +
                '<td class="ca-custom-col-photo">' +
                photoHtml +
                '</td>' +
                '<td class="ca-custom-col-client">' +
                '<div class="ca-custom-row-name">' +
                esc(clientName) +
                '</div>' +
                (clientMeta ? '<div class="admin-muted ca-custom-row-meta">' + esc(clientMeta) + '</div>' : '') +
                visitorLink +
                '</td>' +
                '<td>' +
                leadStatusBadge(lead.display_status || lead.status) +
                '</td>' +
                '<td class="ca-custom-col-order">' +
                '<div class="ca-custom-row-name">' +
        ...
        host.innerHTML =
          '<div class="admin-card ca-custom-table-card"><div class="admin-table-wrap">' +
          '<table class="admin-table ca-custom-table"><thead><tr>' +
