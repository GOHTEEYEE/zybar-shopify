...
var DEFAULT_SHIPPING_METHOD = 'priority';
var SHIPPING_METHOD_KEY = 'zybar.shipping.method';
var CUSTOM_PRODUCT_SLUG = 'custom-led-car-wall-art';
...
function isCustomProductSlug(slug) {
  return String(slug || '').trim() === CUSTOM_PRODUCT_SLUG;
}

function getCustomDesignFeeUSD(catalog, slug) {
  if (!isCustomProductSlug(slug)) return 0;
  var product = getProductEntry(catalog, slug);
  var fee = product && Number(product.customDesignFeeUsd);
  if (Number.isFinite(fee) && fee >= 0) return roundMoney(fee);
  return DEFAULT_CUSTOM_DESIGN_FEE_USD;
}

function getProductBaseUnitPriceUSD(catalog, options) {
  options = options || {};
...
function calculateProductUnitPrice(catalog, options) {
  options = options || {};
  var slug = options.productSlug || options.slug || '';
  var base = getProductBaseUnitPriceUSD(catalog, options);
  return roundMoney(base + getCustomDesignFeeUSD(catalog, slug));
}
...
function repairCartItem(catalog, item) {
  ...
  if (isCustomProductSlug(slug)) item.productType = 'custom';
  var computed = calculateProductUnitPrice(catalog, {
    slug: slug,
    size: size,
    powerType: powerType
  });
  // Never wipe a stored price with $0 (empty catalog / unknown slug).
  if (computed > 0) item.unitPriceUSD = computed;
  if (isCustomProductSlug(slug)) {
    item.customDesignFeeUSD = getCustomDesignFeeUSD(catalog, slug);
    item.baseUnitPriceUSD = getProductBaseUnitPriceUSD(catalog, {
      slug: slug,
      size: size,
      powerType: powerType
    });
...
function createPricingApi(catalog) {
  ...
  return {
    ...
    isCustomProductSlug: isCustomProductSlug,
    getCustomDesignFeeUSD: function (slug) { return getCustomDesignFeeUSD(catalog, slug); },
    getProductBaseUnitPriceUSD: function (o) { return getProductBaseUnitPriceUSD(catalog, o); },
