...
function contentIdsFromSession(session, extras) {
  extras = extras || {};
  const meta = (session && session.metadata) || {};
  const ids = [];
  if (meta.productSlug) ids.push(String(meta.productSlug));

  const fromExtras = Array.isArray(extras.lineItems) ? extras.lineItems : [];
  fromExtras.forEach(function (row) {
    if (row && (row.productSlug || row.slug)) ids.push(String(row.productSlug || row.slug));
  });

  // Legacy sessions only — new checkouts store cart JSON in checkout_snapshots.
  try {
...
async function sendPurchaseFromCheckoutSession(session, extras) {
  ...
  const contentIds = contentIdsFromSession(session, extras);
    ...
    data: [
      {
        ...
        user_data: buildUserData({
          ...
          visitor_id: meta.visitorId || extras.visitor_id || null
        }),
        custom_data: {
          currency: currency,
...
/**
 * PayPal (and other non-Stripe) paid orders — same event_id scheme as browser Pixel:
 * purchase:{sessionId} where sessionId is typically "paypal:{orderId}".
 */
async function sendPurchaseFromPaidOrder(order, extras) {
  order = order || {};
  extras = extras || {};
  if (!order.id) {
  ...
  return sendPurchaseFromCheckoutSession(
    {
      id: String(order.id),
      amount_total: Number(order.amount_cents) || 0,
      currency: order.currency || 'usd',
      customer_email: order.customer_email || null,
      metadata: Object.assign(
        {
          visitorId: order.visitor_id || extras.visitor_id || null,
          quantity: Number.isFinite(quantity) && quantity > 0 ? String(quantity) : undefined
        },
        order.metadata || {}
      )
    },
...
module.exports = {
  ...
  sendPurchaseFromPaidOrder: sendPurchaseFromPaidOrder
};
