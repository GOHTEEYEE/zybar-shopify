...
var DEFAULT_UNATTACHED_DAYS = 7;
var DEFAULT_UNPAID_DAYS = 30;
...
async function cleanupAbandoned(supabase, options) {
  ...
  var unattachedDays = Number(options.unattachedDays);
  var unpaidDays = Number(options.unpaidDays);
  if (!Number.isFinite(unattachedDays) || unattachedDays < 1) {
    unattachedDays = DEFAULT_UNATTACHED_DAYS;
  }
  if (!Number.isFinite(unpaidDays) || unpaidDays < 1) {
    unpaidDays = DEFAULT_UNPAID_DAYS;
  }

  var result = await supabase.rpc('cleanup_abandoned_checkout_snapshots', {
    p_unattached_days: unattachedDays,
...
module.exports = {
  ...
  DEFAULT_UNATTACHED_DAYS: DEFAULT_UNATTACHED_DAYS,
  DEFAULT_UNPAID_DAYS: DEFAULT_UNPAID_DAYS,
