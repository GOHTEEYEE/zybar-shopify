...
function isTestVisitorId(visitorId) {
  return /^v1-/i.test(String(visitorId || ''));
}

/**
 * Enrich visitor rows with session/cart/order/lead/event context.
 * Used by both the general Activity list and status-first queries.
 */
async function enrichVisitorRows(supabase, visitors, options) {
  options = options || {};
  var forceStatus = options.forceStatus || null;
  var cartsByVisitorOverride = options.cartsByVisitor || null;
  ...
  var [sessionsRes, cartsRes, ordersRes, leadsRes, eventsRes] = await Promise.all([
    supabase
      .from('sessions')
      .select(
        'visitor_id,country,traffic_source,utm_source,utm_campaign,device_type,browser,started_at,last_activity_at,user_agent'
      )
      .in('visitor_id', visitorIds)
      .order('started_at', { ascending: false })
      .limit(Math.min(2000, visitorIds.length * 8)),
    cartsByVisitorOverride
      ? Promise.resolve({ data: [], error: null })
      : supabase
          .from('cart_sessions')
          .select(
            'id,visitor_id,status,cart_value_cents,item_count,last_activity_at,created_at,customer_id,metadata'
          )
          .in('visitor_id', visitorIds)
          .order('last_activity_at', { ascending: false })
          .limit(Math.min(2000, visitorIds.length * 5)),
    supabase
      .from('orders')
      .select(
        'id,visitor_id,customer_name,customer_email,customer_phone,country,city,amount_total_cents,product_slug,status,created_at,stripe_session_id'
      )
      .in('visitor_id', visitorIds)
    ...
    supabase
      ...
      .select(
        'email,visitor_id,country,language,discount_code,source,created_at,purchased,order_count,revenue_cents'
      )
      .in('visitor_id', visitorIds)
      .limit(Math.min(1000, visitorIds.length * 2)),
    supabase
      .from('events')
      ...
      .order('created_at', { ascending: false })
      .limit(Math.min(4000, visitorIds.length * 20))
  ]);
  ...
  var latestCart = cartsByVisitorOverride ? Object.assign({}, cartsByVisitorOverride) : {};
  if (!cartsByVisitorOverride) {
    (cartsRes.data || []).forEach(function (c) {
      if (!latestCart[c.visitor_id]) latestCart[c.visitor_id] = c;
    });
  }

  var cartIds = Object.keys(latestCart)
    .map(function (vid) {
      return latestCart[vid] && latestCart[vid].id;
    })
    .filter(Boolean);
  ...
  if (cartIds.length) {
    var itemsRes = await supabase
      ...
      .select('cart_id,product_id,product_name,quantity')
      .in('cart_id', cartIds)
      .limit(2000);
    (itemsRes.data || []).forEach(function (item) {
      if (!productByCart[item.cart_id] && item.product_id) {
        productByCart[item.cart_id] = item.product_id;
      }
    });
  }
  ...
  return visitors.map(function (v) {
    ...
    var status =
      ...
      deriveStatus({
        ...
        last_activity_at: cart.last_activity_at || v.last_seen_at,
        email: (order && order.customer_email) || (lead && lead.email) || cart.customer_id
      });
    var email = (order && order.customer_email) || (lead && lead.email) || cart.customer_id || null;
    var name = (order && order.customer_name) || (email ? String(email).split('@')[0] : null);
    var country =
      (order && order.country) || session.country || v.country || (lead && lead.country) || null;
    var traffic = normalizeSource(session.traffic_source || v.first_traffic_source);
    var productId =
      f.last_product_id ||
      (cart.id && productByCart[cart.id]) ||
      (order && order.product_slug) ||
      null;
    return {
...
/**
 * Status-first lists for funnel stages that are sparse among "recent visitors".
 * Abandoned / added_to_cart / checkout_started must come from cart_sessions,
 * otherwise pagination over recent analytics_visitors hides almost all of them.
 */
async function listActivitiesByCartStatus(supabase, query, cartStatuses, forceStatus) {
  ...
  // Load enough carts to cover this page after collapsing to distinct visitors.
  var fetchLimit = Math.min(1000, Math.max(offset + limit * 5, 200));
  var cartsRes = await supabase
    .from('cart_sessions')
    .select(
      'id,visitor_id,status,cart_value_cents,item_count,last_activity_at,created_at,customer_id,metadata,country',
      { count: 'exact' }
    )
    .in('status', cartStatuses)
    .gte('last_activity_at', range.start)
    .lt('last_activity_at', range.end)
    .order('last_activity_at', { ascending: false })
    .limit(fetchLimit);
  ...
  var cartsByVisitor = {};
  var orderedVisitorIds = [];
  (cartsRes.data || []).forEach(function (cart) {
    if (!cart.visitor_id || isTestVisitorId(cart.visitor_id)) return;
    // Prefer the newest cart; skip empty $0 noise unless it is the only cart.
    if (!cartsByVisitor[cart.visitor_id]) {
      cartsByVisitor[cart.visitor_id] = cart;
      orderedVisitorIds.push(cart.visitor_id);
    }
  });

  // Exact-ish total: distinct non-test visitors among the fetched abandoned set.
  // For large datasets this is approximate unless we fetch all; with current volume it is exact.
  var totalDistinct = orderedVisitorIds.length;
  if ((cartsRes.count || 0) > fetchLimit) {
    // Fall back to count of cart rows when we could not load the full set.
    totalDistinct = cartsRes.count || totalDistinct;
  }

  var pageVisitorIds = orderedVisitorIds.slice(offset, offset + Math.max(limit * 4, 80));
  if (!pageVisitorIds.length) {
    return {
      rows: [],
      total: totalDistinct,
      range: range,
  ...
  var visitorsRes = await supabase
    ...
    .in('visitor_id', pageVisitorIds);
  if (visitorsRes.error) throw visitorsRes.error;

  var visitorMap = {};
  (visitorsRes.data || []).forEach(function (v) {
    visitorMap[v.visitor_id] = v;
  });

  // Carts without analytics_visitors rows still appear (missing join was dropping ~19 abandoned).
  var visitors = pageVisitorIds.map(function (visitorId) {
    var cart = cartsByVisitor[visitorId] || {};
    return (
      visitorMap[visitorId] || {
        visitor_id: visitorId,
        first_seen_at: cart.created_at || cart.last_activity_at || null,
        last_seen_at: cart.last_activity_at || null,
        first_traffic_source: null,
  ...
  var rows = await enrichVisitorRows(supabase, visitors, {
    forceStatus: forceStatus,
    cartsByVisitor: cartsByVisitor
  });

  // Preserve cart recency order.
  var rank = {};
  pageVisitorIds.forEach(function (id, i) {
    rank[id] = i;
  });
  rows.sort(function (a, b) {
    return (rank[a.visitor_id] || 0) - (rank[b.visitor_id] || 0);
  });
  ...
  rows = rows.filter(function (r) {
    ...
    if (search) {
      var blob = [r.email, r.customer_name, r.country, r.current_product, r.visitor_id]
        .join(' ')
        .toLowerCase();
...
async function listActivitiesByPurchased(supabase, query) {
  var range = parseRange(query);
  var limit = Math.min(100, Math.max(1, parseInt(query.limit, 10) || 40));
  var offset = Math.max(0, parseInt(query.offset, 10) || 0);
  var countryFilter = query.country ? String(query.country).toUpperCase() : '';
  ...
  var ordersRes = await supabase
    .from('orders')
    .select(
      'id,visitor_id,customer_name,customer_email,country,amount_total_cents,product_slug,status,created_at,stripe_session_id',
      { count: 'exact' }
    )
  ...
  var ordered = [];
  (ordersRes.data || []).forEach(function (o) {
    if (!o.visitor_id || isTestVisitorId(o.visitor_id)) return;
    if (!byVisitor[o.visitor_id]) {
      byVisitor[o.visitor_id] = o;
      ordered.push(o.visitor_id);
    }
  });
  var pageIds = ordered.slice(0, limit * 2);
  if (!pageIds.length) {
    return {
      rows: [],
  ...
  var visitorsRes = await supabase
    ...
    .in('visitor_id', pageIds);
  if (visitorsRes.error) throw visitorsRes.error;
  var visitorMap = {};
  (visitorsRes.data || []).forEach(function (v) {
    visitorMap[v.visitor_id] = v;
  });
  var visitors = pageIds.map(function (id) {
    var o = byVisitor[id];
    return (
      visitorMap[id] || {
        visitor_id: id,
        ...
        session_count: 0,
        order_count: 1,
        total_spent_cents: o.amount_total_cents || 0
      }
    );
  });

  var rows = await enrichVisitorRows(supabase, visitors, { forceStatus: 'purchased' });
  rows = rows.filter(function (r) {
    if (countryFilter && String(r.country || '').toUpperCase() !== countryFilter) return false;
    if (trafficFilter && r.traffic_source !== trafficFilter) return false;
    if (search) {
...
async function listActivities(supabase, query) {
  ...
  if (statusFilter === 'abandoned') {
    return listActivitiesByCartStatus(supabase, query, ['abandoned'], 'abandoned');
  }
  if (statusFilter === 'added_to_cart') {
    // Current cart stage: active carts (not yet abandoned / checked out).
    return listActivitiesByCartStatus(supabase, query, ['active'], 'added_to_cart');
  }
  if (statusFilter === 'checkout_started') {
    return listActivitiesByCartStatus(supabase, query, ['checkout_started'], 'checkout_started');
  }
  if (statusFilter === 'purchased') {
    return listActivitiesByPurchased(supabase, query);
  }
  ...
  var totalVisitors = countRes.count || 0;

  // When a derived status filter remains (browsing/product_viewed/etc), oversample.
  var oversample = statusFilter ? Math.min(500, Math.max(limit * 12, 120)) : limit;
  var visitorsRes = await supabase
    .from('analytics_visitors')
    .select(
      'visitor_id,first_seen_at,last_seen_at,first_traffic_source,country,device_type,browser,session_count,order_count,total_spent_cents'
    ...
    .range(offset, offset + oversample - 1);
  ...
  rows.forEach(function (r) {
    if (r._orders) ordersByVisitor[r.visitor_id] = r._orders;
    delete r._orders;
  });
  ...
  var total = statusFilter || countryFilter || trafficFilter || search ? rows.length + offset : totalVisitors;
...
/** Fast exact count for dashboard KPIs — ZYBAR automotive leads only. */
async function countEmailLeads(supabase, query) {
  var BrandAnalytics = require('./brand-analytics.js');
  var range = parseRange(query);
  var branded = BrandAnalytics.applyZybarLeadFilters(
    supabase
      .from('newsletter_subscribers')
  ...
  var res = await branded;
  if (!res.error) return res.count || 0;

  // brand column / filter unsupported — hydrate and filter in JS
  var fb = await supabase
    .from('newsletter_subscribers')
    .select('id,source,discount_code')
    .gte('created_at', range.start)
    .lt('created_at', range.end)
    .limit(10000);
  if (fb.error) throw fb.error;
  return BrandAnalytics.filterZybarLeads(fb.data || []).length;
}

/**
 * Count abandoned carts in range. Prefer a direct table filter so the date
 * range is honoured (the list RPC currently ignores start/end).
 */
async function countAbandoned(supabase, query) {
  var BrandAnalytics = require('./brand-analytics.js');
  var range = parseRange(query);
  var cutoffMs = Date.now() - ABANDONED_HOURS * 3600000;
  ...
  var rows = res.data || [];
  var branded = rows.filter(function (row) {
    return String(row.brand || '').toLowerCase() === 'zybar';
  });
  if (branded.length === rows.length && rows.some(function (r) { return r.brand; })) {
    return branded.length;
  }
  var itemsByCart = await BrandAnalytics.loadCartItemsByCartId(
    supabase,
    rows.map(function (r) {
      return r.id;
    })
  );
  return BrandAnalytics.filterZybarCarts(rows, itemsByCart).length;
}
...
async function listEmailLeads(supabase, query) {
  ...
  var res = await supabase
    ...
    .order('created_at', { ascending: false })
    .limit(500);
  if (res.error && /visitor_id|column|brand/i.test(String(res.error.message || ''))) {
    res = await supabase
      .from('newsletter_subscribers')
  ...
  var leads = BrandAnalytics.filterZybarLeads(res.data || []).slice(0, 200);
...
async function listAbandoned(supabase, query) {
  ...
  var res = await supabase.rpc('get_abandoned_carts', {
    p_limit: Math.min(200, parseInt(query.limit, 10) || 50),
    p_offset: Math.max(0, parseInt(query.offset, 10) || 0)
  });
  if (res.error) {
    var fb = await supabase
      .from('cart_sessions')
      .select('id,visitor_id,customer_id,cart_value_cents,item_count,country,last_activity_at,status,created_at,brand')
      .in('status', ['abandoned', 'active', 'checkout_started'])
      .gte('last_activity_at', range.start)
      .order('last_activity_at', { ascending: false })
      .limit(100);
    ...
    carts = BrandAnalytics.filterZybarCarts(carts, itemsByCart).slice(0, 50);
  ...
  return mapped
    .filter(function (c) {
      return BrandAnalytics.isZybarCart(c, c.products);
    })
    .slice(0, 50);
}
...
async function countryAnalytics(supabase, query) {
  ...
  var [sessionsRes, ordersRes] = await Promise.all([
    supabase
      .from('sessions')
      .select('visitor_id,country,landing_page')
      .gte('started_at', range.start)
      .lt('started_at', range.end)
      .limit(10000),
    supabase
      .from('orders')
      .select('visitor_id,country,amount_total_cents,customer_email,status,product_slug,line_items')
      .gte('created_at', range.start)
      .lt('created_at', range.end)
      .limit(5000)
  ]);
  var map = {};
  BrandAnalytics.filterZybarSessions(sessionsRes.data || []).forEach(function (s) {
  ...
  BrandAnalytics.filterZybarOrders(ordersRes.data || []).forEach(function (o) {
...
async function trafficAnalytics(supabase, query) {
  ...
  var [sessionsRes, eventsRes, ordersRes] = await Promise.all([
    supabase
      .from('sessions')
      .select('visitor_id,traffic_source,started_at,landing_page')
      .gte('started_at', range.start)
      .lt('started_at', range.end)
      .limit(10000),
    supabase
      .from('events')
      .select('visitor_id,event_type,session_id,page_url,product_id,collection_id')
      .gte('created_at', range.start)
      .lt('created_at', range.end)
    ...
    supabase
      .from('orders')
      .select('visitor_id,amount_total_cents,status,product_slug,line_items')
      .gte('created_at', range.start)
      .lt('created_at', range.end)
      .limit(5000)
  ]);
  ...
  BrandAnalytics.filterZybarEvents(eventsRes.data || []).forEach(function (ev) {
...
module.exports = {
  ...
  countEmailLeads: countEmailLeads,
