...
/** Fast exact count for dashboard KPIs — no row hydration or order joins. */
async function countEmailLeads(supabase, query) {
  ...
  var q = supabase
    ...
    .gte('created_at', range.start)
    .lt('created_at', range.end);
  if (query && query.excludeLuneva) {
    q = q.not('source', 'ilike', '%luneva%');
  }
  var res = await q;
  if (res.error) {
    // Older schemas may lack `source`; fall back to unfiltered count.
    if (/source|column/i.test(String(res.error.message || ''))) {
      res = await supabase
        .from('newsletter_subscribers')
        .select('id', { count: 'exact', head: true })
        .gte('created_at', range.start)
...
async function countAbandoned(supabase, query) {
  ...
  if (query && query.excludeLuneva) {
    var detailed = await supabase
      .from('cart_sessions')
      .select('id, status, last_activity_at, cart_session_items(product_id)')
    ...
    if (!detailed.error && Array.isArray(detailed.data)) {
      return detailed.data.filter(function (cart) {
        var items = cart.cart_session_items || [];
        if (!items.length) return true;
        return !items.some(function (item) {
          return String((item && item.product_id) || '').indexOf('luneva-') === 0;
        });
      }).length;
    }
  }
  ...
  var res = await supabase
    ...
    .lt('last_activity_at', cutoffIso);
  if (res.error) {
    // If status vocabulary differs, fall back to abandoned-only.
