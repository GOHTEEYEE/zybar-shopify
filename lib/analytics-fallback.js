...
async function sumAddToCartQuantity(supabase, range) {
  ...
  return data.reduce(function (sum, ev) { return sum + eventQuantity(ev); }, 0);
}
...
async function overviewFallback(supabase, range) {
  ...
  return {
    ...
    conversion_rate: uniqueVisitors > 0 ? Number(((orders / uniqueVisitors) * 100).toFixed(2)) : 0,
...
async function funnelFallback(supabase, range) {
  ...
  const counts = await Promise.all([
    countDistinct(supabase, 'sessions', 'visitor_id', { startCol: 'started_at', gte: range.start, lt: range.end }),
    countDistinct(supabase, 'events', 'visitor_id', { startCol: 'created_at', gte: range.start, lt: range.end }),
    countDistinct(supabase, 'events', 'visitor_id', { startCol: 'created_at', gte: range.start, lt: range.end }),
    countDistinct(supabase, 'events', 'visitor_id', { startCol: 'created_at', gte: range.start, lt: range.end }),
    countRows(supabase, 'orders', { created_at_gte: range.start, created_at_lt: range.end })
  ]);

  // Refine event-based steps with filters
  const { data: events } = await supabase
    .from('events')
    .select('event_type, visitor_id')
    .gte('created_at', range.start)
    .lt('created_at', range.end)
    .in('event_type', ['product_view', 'add_to_cart', 'begin_checkout'])
    .limit(10000);

  const pv = {}; const atc = {}; const chk = {};
  (events || []).forEach(function (ev) {
    ...
    if (ev.event_type === 'begin_checkout') chk[ev.visitor_id] = true;
  });
  counts[1] = Object.keys(pv).length;
  counts[2] = Object.keys(atc).length;
  counts[3] = Object.keys(chk).length;
  ...
  return steps.map(function (s, i) {
    ...
    return {
      ...
      count: cnt,
      order: s.order,
      rate_from_previous: prev ? Number(((cnt / prev) * 100).toFixed(2)) : 100
    };
  });
}
...
async function trendsFallback(supabase, range, granularity) {
  ...
  const [{ data: events }, { data: sessions }, { data: orders }] = await Promise.all([
    supabase.from('events').select('event_type,created_at,quantity,metadata').gte('created_at', range.start).lt('created_at', range.end).limit(10000),
  ...
  (events || []).forEach(function (ev) {
    ...
    if (ev.event_type === 'begin_checkout') checkout[d] = (checkout[d] || 0) + 1;
  });
...
async function productsFallback(supabase, range) {
  const { data: events } = await supabase
    .from('events')
    .select('event_type,product_id,quantity,metadata')
    .gte('created_at', range.start)
    .lt('created_at', range.end)
...
async function distributionsFallback(supabase, range) {
  const { data: sessions } = await supabase
    .from('sessions')
    .select('country,device_type,browser,traffic_source,visitor_id')
    .gte('started_at', range.start)
    .lt('started_at', range.end)
    .limit(10000);
...
async function realtimeFallback(supabase) {
  ...
  const [{ data: sessions }, { data: carts }, { data: events }] = await Promise.all([
    supabase.from('sessions').select('visitor_id,id,last_activity_at').gte('last_activity_at', since).limit(500),
    ...
    supabase.from('events').select('event_type,product_id,visitor_id,page_url,created_at').gte('created_at', since).order('created_at', { ascending: false }).limit(20)
  ]);

  const visitors = {};
  (sessions || []).forEach(function (s) { if (s.visitor_id) visitors[s.visitor_id] = true; });
...
async function geoTrafficFallback(supabase, range) {
  const [{ data: sessions }, { data: events }, { data: orders }] = await Promise.all([
    supabase
      .from('sessions')
      .select('id,visitor_id,started_at,last_activity_at,country,traffic_source,utm_source,utm_campaign')
      .gte('started_at', range.start)
      .lt('started_at', range.end)
      .limit(10000),
    supabase
    ...
    supabase
      .from('orders')
      .select('visitor_id,amount_total_cents,status,created_at')
      .gte('created_at', range.start)
      .lt('created_at', range.end)
      .limit(10000)
  ]);
