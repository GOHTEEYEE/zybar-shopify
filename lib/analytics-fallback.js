...
async function countDistinct(supabase, table, column, filters, lunevaSets) {
  const selectCols =
    table === 'sessions'
      ? column === 'id'
        ? 'id,visitor_id,landing_page'
        : column + ',id,visitor_id,landing_page'
      : table === 'events'
  ...
  const { data, error } = await supabase
    ...
    .limit(10000);
  if (error || !Array.isArray(data)) return 0;
  const sets = lunevaSets || { sessionIds: {}, visitorIds: {} };
  const set = {};
  data.forEach(function (row) {
    if (table === 'sessions') {
      if (!BrandAnalytics.isZybarSessionStrict(row, sets.sessionIds, sets.visitorIds)) return;
    }
...
async function fetchLunevaIdentitySets(supabase, range) {
  const { data } = await supabase
    .from('events')
    .select('session_id,visitor_id,page_url,product_id,collection_id')
    .or(BrandAnalytics.lunevaEventFilter())
    .gte('created_at', range.start)
    .lt('created_at', range.end)
    .limit(10000);
  return BrandAnalytics.buildLunevaIdentitySets(data || []);
}
...
async function overviewFallback(supabase, range) {
  ...
  const lunevaSets = await fetchLunevaIdentitySets(supabase, range);
  ...
  const [
    ...
    countDistinct(
      ...
      'sessions',
      'visitor_id',
      { startCol: 'started_at', gte: range.start, lt: range.end },
      lunevaSets
    ),
    countDistinct(
      supabase,
  ...
  const [{ data: orderRows }, { data: sessionRows }] = await Promise.all([
    ...
    supabase
      .from('sessions')
      .select('id,visitor_id,started_at,last_activity_at,landing_page')
  ...
  var durations = [];
  BrandAnalytics.filterZybarSessionsStrict(
    sessionRows || [],
    lunevaSets.sessionIds,
    lunevaSets.visitorIds
  ).forEach(function (s) {
  ...
  const zybarVisitorIds = {};
  BrandAnalytics.filterZybarSessionsStrict(
    sessionRows || [],
    lunevaSets.sessionIds,
    lunevaSets.visitorIds
  ).forEach(function (s) {
    if (s.visitor_id) zybarVisitorIds[s.visitor_id] = true;
  });
  ...
  if (Array.isArray(visitorRows)) {
    visitorRows.forEach(function (v) {
      if (!zybarVisitorIds[v.visitor_id]) return;
...
async function trendsFallback(supabase, range, granularity) {
  ...
  const [{ data: events }, { data: sessions }, { data: orders }, { data: leads }, { data: carts }, { data: customLeads }] =
    await Promise.all([
      supabase
        .from('events')
        .select('event_type,created_at,quantity,metadata,page_url,product_id,collection_id,session_id,visitor_id')
        .gte('created_at', range.start)
        .lt('created_at', range.end)
        .limit(10000),
      supabase
        .from('sessions')
        .select('id,visitor_id,started_at,landing_page')
        .gte('started_at', range.start)
        .lt('started_at', range.end)
        .limit(10000),
      supabase
      ...
      supabase
        .from('cart_sessions')
        .select('created_at,abandoned_at,last_activity_at,status,recovery_status,purchased_at')
        .gte('created_at', range.start)
        .lt('created_at', range.end)
        .limit(10000),
      supabase
  ...
  const lunevaSets = BrandAnalytics.buildLunevaIdentitySets(events || []);
  const visitors = {};
  const sessionCounts = {};
  BrandAnalytics.filterZybarSessionsStrict(
    sessions || [],
...
async function distributionsFallback(supabase, range) {
  ...
  const { data: sessions } = await supabase
    .from('sessions')
    .select('id,country,device_type,browser,traffic_source,visitor_id,landing_page')
    .gte('started_at', range.start)
    .lt('started_at', range.end)
    .limit(10000);
...
async function realtimeFallback(supabase) {
  ...
  const [{ data: sessions }, { data: carts }, { data: events }, { data: lunevaEvents }] = await Promise.all([
    supabase.from('sessions').select('visitor_id,id,last_activity_at,landing_page').gte('last_activity_at', since).limit(500),
    supabase.from('cart_sessions').select('visitor_id,status,item_count,last_activity_at').gte('last_activity_at', new Date(Date.now() - 30 * 60 * 1000).toISOString()).limit(500),
    supabase.from('events').select('event_type,product_id,visitor_id,page_url,collection_id,session_id,created_at').gte('created_at', since).order('created_at', { ascending: false }).limit(50),
    supabase
      .from('events')
      .select('session_id,visitor_id,page_url,product_id,collection_id')
      .or(BrandAnalytics.lunevaEventFilter())
  ...
  const lunevaSets = BrandAnalytics.buildLunevaIdentitySets(lunevaEvents || []);
  const visitors = {};
  BrandAnalytics.filterZybarSessionsStrict(
    sessions || [],
    lunevaSets.sessionIds,
  ...
  let activeCarts = 0; let checkoutUsers = {};
  (carts || []).forEach(function (c) {
  ...
  return {
    ...
    active_sessions: BrandAnalytics.filterZybarSessionsStrict(
      sessions || [],
      lunevaSets.sessionIds,
      lunevaSets.visitorIds
    ).length,
...
async function geoTrafficFallback(supabase, range) {
  ...
  const fullLunevaSets = await fetchLunevaIdentitySets(supabase, range);
  const sessionList = BrandAnalytics.filterZybarSessionsStrict(
    sessions || [],
    fullLunevaSets.sessionIds,
    fullLunevaSets.visitorIds
  );
