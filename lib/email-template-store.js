...
function wrapSimpleHtml(inner, vars) {
  ...
  return (
    ...
    '</td></tr>' +
    '<tr><td align="center" style="padding:0 32px 32px;font-family:Helvetica,Arial,sans-serif;font-size:11px;line-height:1.8;color:rgba(255,255,255,0.38);">' +
    storeName +
    ' · Tokyo, Japan<br/>' +
    '<a href="{{unsubscribe_url}}" style="color:rgba(255,255,255,0.38);text-decoration:underline;">Unsubscribe</a>' +
    '</td></tr></table></td></tr></table></body></html>'
  );
}

/**
 * Templates carry {{unsubscribe_url}} rather than a finished link, so the
 * recipient-specific URL is injected here — after store links are decorated,
 * which keeps a member_token off the unsubscribe link.
 */
function finalizeRendered(rendered, vars) {
  if (!rendered || !rendered.html) return rendered;
  const decorated = MemberPricing.decorateStoreLinks(
    rendered.html,
    vars.storeUrl || CodeTemplates.DEFAULT_STORE_URL,
    vars.memberCredential
  );
  return Object.assign({}, rendered, {
    html: Unsubscribe.applyUrlToHtml(decorated, vars.unsubscribeUrl)
  });
}
  ...
  try {
    if (CodeTemplates.getTemplateDefinition(templateKey)) {
      ...
      if (!dbRow || !dbRow.html_body || dbRow.source === 'code') {
        return finalizeRendered(codeRendered, vars);
      }
      // Use DB only when the row was actually edited after seeding; untouched
      // seed stubs (which contain {{vars}}) must not shadow rich code templates.
      if (dbRow.updated_at && dbRow.created_at && dbRow.updated_at !== dbRow.created_at) {
        ...
        return finalizeRendered({ subject: subject, html: html }, vars);
      }
      return finalizeRendered(codeRendered, vars);
    }
  } catch (e) {
