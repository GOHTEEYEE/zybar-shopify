 ...
 * report opens or clicks, so they are excluded from the Completed queue view.
 */
const EMAIL_TRACKING_START_MS = Date.parse('2026-07-25T03:32:00.000Z');

function isTrackableSend(row) {
  if (!row || !row.executed_at) return false;
  const at = Date.parse(String(row.executed_at).replace(' ', 'T'));
...
async function getOverview(supabase) {
  ...
  let leadsRes = await supabase
    ...
    .select('id,source,discount_code,brand')
    .or('is_test.is.null,is_test.eq.false')
    .limit(20000);
  let leadsTodayRes = await supabase
    .from('newsletter_subscribers')
    .select('id,source,discount_code,brand')
    .gte('created_at', todayStart)
    .or('is_test.is.null,is_test.eq.false')
    .limit(5000);
  let neverEnrolledRes = await supabase
    .from('newsletter_subscribers')
    .select('id,source,discount_code,brand')
  ...
  if (leadsRes.error && /brand|column/i.test(String(leadsRes.error.message || ''))) {
    leadsRes = await supabase
      .from('newsletter_subscribers')
      .select('id,source,discount_code')
      .or('is_test.is.null,is_test.eq.false')
      .limit(20000);
    leadsTodayRes = await supabase
      .from('newsletter_subscribers')
      .select('id,source,discount_code')
      .gte('created_at', todayStart)
      .or('is_test.is.null,is_test.eq.false')
      .limit(5000);
    neverEnrolledRes = await supabase
      .from('newsletter_subscribers')
      .select('id,source,discount_code')
      .is('current_journey_id', null)
      .or('is_test.is.null,is_test.eq.false')
  ...
  const [enrollRes, queueRes, ordersRes] = await Promise.all([
    supabase.from('lead_journeys').select('id, status, journey_id'),
    supabase
      .from('action_queue')
      .select('id, status, scheduled_at, executed_at, journey_id, action_type, template_id, recipient, opened_at, clicked_at'),
    supabase
      .from('orders')
      .select('id, amount_total_cents, customer_email, created_at, status, product_slug, line_items')
      .limit(5000)
  ]);
  ...
  const totalLeads = BrandAnalytics.filterZybarLeads(leadsRes.data || []).length;
  const subscribersToday = BrandAnalytics.filterZybarLeads(leadsTodayRes.data || []).length;
  const neverEnrolled = BrandAnalytics.filterZybarLeads(neverEnrolledRes.data || []).length;
  ...
  const orders = BrandAnalytics.filterZybarOrders(ordersRes.data || []);
  ...
  const emailsSent = queue.completed_trackable || 0;
  ...
  // Lead categories by current journey (Welcome / Cart / Purchase)
  const categoryDefs = [
    {
      key: 'welcome',
      label: 'Welcome',
      journey_key: 'welcome_journey',
      href: '#marketing/audience?journey=welcome_journey'
    },
    {
      key: 'cart',
      label: 'Add to Cart',
      journey_key: 'cart_journey',
      href: '#marketing/audience?journey=cart_journey'
    },
    {
      key: 'purchase',
      label: 'Purchase',
      journey_key: 'customer_journey',
      href: '#marketing/audience?journey=customer_journey'
    },
    {
      key: 'win_back',
      label: 'Win Back',
      journey_key: 'win_back_journey',
      href: '#marketing/audience?journey=win_back_journey'
    }
  ];

  const [coreJourneysRes, currentLeadsRes] = await Promise.all([
    supabase
      .from('journeys')
      .select('id,journey_key,name')
      .in('journey_key', categoryDefs.map(function (c) {
        return c.journey_key;
      })),
    supabase
      .from('newsletter_subscribers')
      .select('id,current_journey_id,journey_status')
      .not('current_journey_id', 'is', null)
      .or('is_test.is.null,is_test.eq.false')
      .limit(5000)
  ]);
  if (coreJourneysRes.error) throw coreJourneysRes.error;
  if (currentLeadsRes.error) throw currentLeadsRes.error;

  const journeyMetaById = {};
  const journeyIdByKey = {};
  (coreJourneysRes.data || []).forEach(function (j) {
    journeyMetaById[j.id] = j;
    journeyIdByKey[j.journey_key] = j.id;
  });

  const byKey = {};
  categoryDefs.forEach(function (c) {
    byKey[c.journey_key] = { current: 0, active: 0, completed: 0 };
  });
  let otherCurrent = 0;
  (currentLeadsRes.data || []).forEach(function (lead) {
    const meta = journeyMetaById[lead.current_journey_id];
    const key = meta && meta.journey_key;
    if (!key || !byKey[key]) {
      otherCurrent += 1;
      return;
    }
    ...
    if (lead.journey_status === 'completed') byKey[key].completed += 1;
  });

  // Ever enrolled (history) for context
  const enrollByJourneyId = {};
  (enrollRes.data || []).forEach(function (row) {
    if (!enrollByJourneyId[row.journey_id]) {
      enrollByJourneyId[row.journey_id] = { waiting: 0, completed: 0, cancelled: 0, total: 0 };
    }
    enrollByJourneyId[row.journey_id].total += 1;
    if (enrollByJourneyId[row.journey_id][row.status] != null) {
      enrollByJourneyId[row.journey_id][row.status] += 1;
    }
  });

  const journey_categories = categoryDefs.map(function (c) {
    const jid = journeyIdByKey[c.journey_key];
    const hist = jid ? enrollByJourneyId[jid] || { waiting: 0, completed: 0, cancelled: 0, total: 0 } : {
      waiting: 0,
      completed: 0,
      cancelled: 0,
      total: 0
    };
    const cur = byKey[c.journey_key] || { current: 0, active: 0, completed: 0 };
    return {
      key: c.key,
      label: c.label,
      ...
      current: cur.current,
      active: cur.active,
      completed_current: cur.completed,
      ever_enrolled: hist.total,
      waiting_history: hist.waiting,
      completed_history: hist.completed,
      cancelled_history: hist.cancelled
    };
  });
  ...
  return {
    kpis: {
      ...
      welcome_leads: byKey.welcome_journey ? byKey.welcome_journey.current : 0,
      cart_leads: byKey.cart_journey ? byKey.cart_journey.current : 0,
      purchase_leads: byKey.customer_journey ? byKey.customer_journey.current : 0
    },
    lifecycle: [
    ...
    journey_categories: journey_categories,
    other_in_journey: otherCurrent,
    upcoming: {
...
async function getAudience(supabase, options) {
  ...
  const journeyFilter = String(options.journey || options.journey_key || '').trim().toLowerCase();
  ...
  if (journeyFilter) {
    const jr = await supabase
      .from('journeys')
      .select('id')
      .eq('journey_key', journeyFilter)
      .maybeSingle();
    if (jr.error) throw jr.error;
    journeyIdFilter = jr.data && jr.data.id ? jr.data.id : '__none__';
  }
  ...
  if (journeyIdFilter === '__none__') query = query.eq('id', '00000000-0000-0000-0000-000000000000');
  else if (journeyIdFilter) query = query.eq('current_journey_id', journeyIdFilter);

  const result = await query;
  if (result.error) throw result.error;

  const BrandAnalytics = require('./brand-analytics.js');
  const filtered = BrandAnalytics.filterZybarLeads(result.data || []);
