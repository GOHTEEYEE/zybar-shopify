...
const Header = defineComponent(
  {
    ...
    optional: [
      ...
      { name: 'width', type: 'number', default: 132, description: 'Logo display width in px.' },
      {
        name: 'textLabel',
        type: 'string',
        description: 'If set, render a text wordmark instead of a logo image (used by LUNEVA).'
      }
    ],
    reusability:
  ...
  function renderHeader(options) {
    ...
    const textLabel = options.textLabel ? String(options.textLabel).trim() : '';
    const inner = textLabel
      ? '<span style="font-family:' +
        BRAND.fontDisplay +
        ';font-size:28px;letter-spacing:0.18em;color:' +
        BRAND.white +
        ';font-weight:400;">' +
        esc(textLabel) +
        '</span>'
...
const ImageBlock = defineComponent(
  {
    ...
    name: 'Image Block',
    purpose:
      'Full-bleed lifestyle or craft image with an optional one-line caption. Primary visual storytelling unit for Welcome Journey days.',
    inputs: [
      { name: 'href', type: 'url', description: 'Click target for the image.' },
      { name: 'imageUrl', type: 'url', description: 'Absolute image URL.' }
    ],
    optional: [
      { name: 'alt', type: 'string', default: '', description: 'Alt text.' },
      { name: 'caption', type: 'string', description: 'Quiet caption under the image.' },
      { name: 'height', type: 'number', default: 360, description: 'Hint height.' }
    ],
    reusability: 'Welcome Day 1–5 lifestyle and craft frames. Prefer over Hero when stacking multiple images.'
  },
  function renderImageBlock(options) {
    options = options || {};
    const href = esc(options.href);
    const imageUrl = esc(options.imageUrl);
    const alt = esc(options.alt || '');
...
const SoftPanel = defineComponent(
  {
    id: 'soft_panel',
    name: 'Soft Panel',
    purpose:
      'Light editorial band for contrast against dark email chrome. Use for short emotional lines, not long copy.',
    inputs: [],
    optional: [
      { name: 'eyebrow', type: 'string' },
      { name: 'headline', type: 'string' },
      { name: 'body', type: 'html-string' }
    ],
    reusability: 'Welcome Journey contrast moments (craft, invitation, philosophy).'
  },
  function renderSoftPanel(options) {
    options = options || {};
    return (
      '<tr><td style="padding:0;">' +
      '<table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" bgcolor="#f4f2ed" style="background:#f4f2ed;">' +
      '<tr><td align="center" class="px" style="padding:44px ' +
      BRAND.gutter +
      'px;text-align:center;">' +
      (options.eyebrow
        ? '<div style="font-family:' +
          BRAND.fontBody +
          ';font-size:11px;letter-spacing:0.28em;text-transform:uppercase;color:#8a8680;padding-bottom:14px;">' +
          esc(options.eyebrow) +
          '</div>'
        : '') +
      (options.headline
        ? '<div class="h2" style="font-family:' +
          BRAND.fontDisplay +
          ';font-size:26px;line-height:1.3;color:#161513;padding-bottom:12px;">' +
          esc(options.headline) +
          '</div>'
        : '') +
      (options.body
        ? '<div style="font-family:' +
          BRAND.fontBody +
          ';font-size:15px;line-height:1.65;color:#5c5852;max-width:420px;margin:0 auto;">' +
          options.body +
          '</div>'
        : '') +
      '</td></tr></table></td></tr>'
...
const CollectionCard = defineComponent(
  {
    ...
    purpose:
      'Identity collection block: one hero image, one sentence, one text CTA. Used in Find Your Style.',
    inputs: [
    ...
    reusability: 'Welcome Day 3 and future collection emails.'
  },
  function renderCollectionCard(options) {
    options = options || {};
    return (
      ...
      '" target="_blank">' +
      '<img src="' +
      esc(options.imageUrl) +
      '" width="' +
      BRAND.width +
      '" alt="' +
      esc(options.alt || options.title || '') +
      '" class="hero-img" style="display:block;width:100%;height:auto;background:' +
      BRAND.cardSoft +
      ';" />' +
      '</a>' +
      '<table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0">' +
      '<tr><td align="center" class="px" style="padding:22px ' +
      BRAND.gutter +
      'px 36px;text-align:center;">' +
      '<div style="font-family:' +
      BRAND.fontBody +
      ';font-size:11px;letter-spacing:0.28em;text-transform:uppercase;color:' +
      BRAND.faint +
      ...
      '<div style="font-family:' +
      BRAND.fontDisplay +
      ';font-size:20px;line-height:1.4;color:' +
      BRAND.text +
      ';padding-bottom:16px;max-width:420px;margin:0 auto;">' +
      esc(options.line) +
      '</div>' +
      '<a href="' +
      esc(options.href) +
...
const QuoteWithImage = defineComponent(
  {
    id: 'quote_with_image',
    name: 'Quote With Image',
    purpose: 'Collector story: lifestyle photo above a short quote and attribution.',
    inputs: [
      { name: 'href', type: 'url' },
      { name: 'imageUrl', type: 'url' },
    ...
    optional: [{ name: 'alt', type: 'string' }, { name: 'caption', type: 'string' }],
    reusability: 'Welcome Day 4 Collector Stories.'
  },
  function renderQuoteWithImage(options) {
    options = options || {};
    return (
      ImageBlock.render({
        href: options.href,
      ...
      Review.render({
        ...
        'padding:36px ' + BRAND.gutter + 'px 28px;border-top:1px solid ' + BRAND.line,
        'padding:8px ' + BRAND.gutter + 'px 40px;border-top:none'
      )
    );
  }
);
