...
async function syncFromPaidSession(supabase, session, orderRow) {
  ...
  var lineItems = Array.isArray(orderRow.line_items) ? orderRow.line_items : [];

  if (!lineItems.length) {
    try {
      var CheckoutSnapshots = require('./checkout-snapshots.js');
      var resolved = await CheckoutSnapshots.resolveLineItemsForSession(supabase, session);
