...
const METRIC_META = {
  ...
  custom_made_leads: {
    ...
    description: 'Custom Made funnel uploads and progress.',
...
async function getMetricSummary(supabase, key, range, filters) {
  ...
  if (key === 'email_leads') {
    ...
    return {
      cards: [
        ...
        card('Custom Made', bySource.custom_made || 0),
  ...
  if (key === 'custom_made_leads') {
    const data = await CustomLeads.list(supabase, {
      preset: 'custom',
      start: range.start,
      end: range.end,
      limit: 200,
    ...
    rows.forEach(function (r) {
      const s = r.display_status || r.status || 'started';
      byStatus[s] = (byStatus[s] || 0) + 1;
      if (r.customer_email) withEmail += 1;
    });
    return {
      cards: [
        card('Custom Made Leads', data.total != null ? data.total : rows.length),
        card('With Email', withEmail),
        card('Configured+', (byStatus.configured || 0) + (byStatus.added_to_cart || 0) + (byStatus.checkout_started || 0) + (byStatus.purchased || 0)),
        card('Purchased', byStatus.purchased || 0)
      ],
      breakdowns: {
        statuses: Object.keys(byStatus).map(function (k) {
    ...
    return {
      ...
      rows: rows.map(function (r) {
        return {
          ...
          client: r.client_label || r.customer_name || 'Anonymous',
