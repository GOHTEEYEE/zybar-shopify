...
/** LUNEVA reuses ZYBAR size codes for kit tiers — never show raw 30x45/40x60 to admins. */
function formatKitLabel(raw) {
  const s = String(raw || '').trim();
  if (!s) return null;
  const key = s.toLowerCase().replace(/\s+/g, '');
  if (key === '30x45' || key === 'lights' || key === 'lightingeffects') {
    return 'Lighting effects';
  }
  if (
    key === '40x60' ||
    key === 'full' ||
    key === 'lighting+mechanicalbutterfly' ||
    key === 'lightingeffects+mechanicalbutterfly'
  ) {
    return 'Lighting effects + Mechanical butterfly';
  }
  // Already a human kit title from cart sizeLabel.
  if (/lighting|mechanical|butterfly|kit/i.test(s) && !/\d+\s*x\s*\d+/i.test(s)) {
    return s;
  }
  // Hide leftover dimension-looking codes.
  if (/^\d+\s*x\s*\d+(\s*cm)?$/i.test(s)) return null;
  return s;
}

function kitFromEventMetadata(meta) {
  if (!meta || typeof meta !== 'object') return null;
  return (
    formatKitLabel(meta.sizeLabel) ||
    formatKitLabel(meta.kit) ||
    formatKitLabel(meta.size) ||
    null
  );
}

function composeProductDisplay(name, kit) {
  const base = formatProductLabel(name) || null;
  const kitLabel = formatKitLabel(kit);
  if (!base) return kitLabel;
  if (!kitLabel) return base;
  if (String(base).toLowerCase().indexOf(kitLabel.toLowerCase()) !== -1) return base;
  return base + ' · ' + kitLabel;
}
...
function eventTimelineLabel(ev) {
  ...
  const kit = kitFromEventMetadata(ev && ev.metadata);
  ...
  if (type === 'checkout_email_filled') return 'Checkout email filled';
  if (type === 'checkout_contact_filled') return 'Checkout name filled';
  if (type === 'checkout_address_partial') return 'Checkout address started';
  if (type === 'checkout_address_filled') return 'Checkout details complete';
...
async function fetchLunevaLeads(supabase, range) {
  let query = supabase
    .from('newsletter_subscribers')
    .select(
      'id,email,source,brand,discount_code,status,created_at,visitor_id,used_discount'
    )
    .gte('created_at', range.start)
    .lt('created_at', range.end)
    .order('created_at', { ascending: false })
  ...
  if (result.error && /brand|column/i.test(String(result.error.message || ''))) {
    const legacy = await supabase
      ...
      .or('source.ilike.%luneva%,discount_code.eq.LUNEVA5')
      .gte('created_at', range.start)
      .lt('created_at', range.end)
      .order('created_at', { ascending: false })
      .limit(2000);
...
function leadSourcePriority(source, status) {
  if (status === 'purchased' || source === 'purchase') return 40;
  if (source === 'checkout_draft' || status === 'details_filled') return 30;
  if (
    source === 'checkout_email' ||
    source === 'luneva_checkout' ||
    status === 'checkout_lead'
  ) {
    return 20;
  }
  if (String(source || '').indexOf('luneva_popup') === 0 || status === 'email_marketing') {
    return 10;
  }
  return 5;
}
...
async function getCustomers(supabase, range) {
  const [orders, events, leads] = await Promise.all([
    fetchLunevaOrders(supabase, range),
    fetchLunevaEvents(supabase, range),
    fetchLunevaLeads(supabase, range)
  ]);
  const byEmail = {};
  ...
  function applySource(row, source, status) {
    if (!row) return;
    const nextSource = source || row.source;
    const nextStatus = status || row.status;
    if (leadSourcePriority(nextSource, nextStatus) >= leadSourcePriority(row.source, row.status)) {
  ...
  (leads || []).forEach(function (lead) {
    ...
    if (!row) return;
    const src = String(lead.source || 'luneva_popup');
    if (src.indexOf('luneva_popup') === 0) {
      applySource(row, src, 'email_marketing');
    } else if (src === 'luneva_checkout' || src === 'checkout_email') {
      applySource(row, src, 'checkout_lead');
    } else if (src.indexOf('luneva_purchase') === 0 || src === 'purchase') {
      applySource(row, 'purchase', 'purchased');
    } else {
      applySource(row, src, 'email_marketing');
    }
    if (!row.last_seen_at || String(lead.created_at) > String(row.last_seen_at)) {
      row.last_seen_at = lead.created_at;
    }
  });

  (events || []).forEach(function (ev) {
    if (
      ev.event_type !== 'email_submitted' &&
      ev.event_type !== 'newsletter_signup' &&
      ev.event_type !== 'checkout_email_filled' &&
      ev.event_type !== 'checkout_contact_filled' &&
      ev.event_type !== 'checkout_address_partial' &&
      ev.event_type !== 'checkout_address_filled'
    ) {
      return;
    }
    const email =
    ...
    if (ev.metadata && ev.metadata.phone) row.phone = String(ev.metadata.phone);
    if (ev.metadata && ev.metadata.country) row.country = String(ev.metadata.country);
    if (ev.metadata && ev.metadata.stage) row.checkout_stage = String(ev.metadata.stage);

    if (ev.event_type === 'email_submitted' || ev.event_type === 'newsletter_signup') {
      const metaSource =
        (ev.metadata && ev.metadata.source) ||
        (ev.metadata && ev.metadata.intent
          ? 'luneva_popup:' + ev.metadata.intent
          : 'luneva_popup');
      applySource(row, metaSource, 'email_marketing');
    } else if (ev.event_type.indexOf('checkout_') === 0) {
      applySource(
        row,
        'checkout_draft',
        ev.event_type === 'checkout_address_filled' ? 'details_filled' : 'checkout_lead'
      );
    }
...
function buildVisitorMap(sessions, events, orders) {
  ...
  (events || []).forEach(function (ev) {
    ...
    if (ev.event_type === 'add_to_cart' && productId) {
      ...
      row.cart_kit = kitFromEventMetadata(ev.metadata);
    }
  ...
  (orders || []).forEach(function (order) {
    ...
    if (!row) return;
    if (order.customer_email) {
      row.email = String(order.customer_email).trim().toLowerCase();
      row.has_email = true;
    }
    if (order.customer_name) row.name = order.customer_name;
  ...
  return Object.keys(map).map(function (visitorId) {
    ...
    if (row.email) row.has_email = true;
    row.has_email = !!row.has_email;
    ...
    row.cart_kit = formatKitLabel(row.cart_kit);
    row.product_label = composeProductDisplay(rawProduct || row.product_id, row.cart_kit);
...
async function getVisitors(supabase, range, query) {
  ...
  const stageFilter = String(opts.stage || opts.action || '')
    .trim()
    .toLowerCase();
  ...
  if (stageFilter && stageFilter !== 'all') {
    rows = rows.filter(function (row) {
      if (stageFilter === 'add_to_cart' || stageFilter === 'atc' || stageFilter === 'cart') {
        return (row.add_to_cart || 0) > 0;
      }
      if (stageFilter === 'checkout') {
        return (row.begin_checkout || 0) > 0 || (row.orders || 0) > 0;
      }
      if (stageFilter === 'purchase' || stageFilter === 'purchased') {
        return (row.orders || 0) > 0 || row.status === 'purchased';
      }
      if (stageFilter === 'email') {
        return !!(row.has_email || row.email);
      }
      return true;
    });
  }
...
async function getVisitorDetail(supabase, visitorId) {
  ...
  const [sessionsRes, eventsRes, ordersRes, leadsRes, visitorRes, cartsRes] = await Promise.all([
    supabase
      .from('sessions')
      .select('*')
      .eq('visitor_id', id)
    ...
    supabase
      .from('cart_sessions')
      .select('id,status,metadata,country,last_activity_at,brand,cart_value_cents')
      .eq('visitor_id', id)
  ...
  const carts = (cartsRes && !cartsRes.error ? cartsRes.data : null) || [];
  const lunevaCarts = carts.filter(function (cart) {
    return BrandAnalytics.isLunevaCart(cart) || BrandAnalytics.inferCartBrand(cart) === 'luneva';
  });
  const cartWithDraft =
    (lunevaCarts.length ? lunevaCarts : carts).find(function (cart) {
      return cart && cart.metadata && cart.metadata.checkout_draft;
    }) || null;
  const checkoutDraft =
    cartWithDraft && cartWithDraft.metadata && cartWithDraft.metadata.checkout_draft
      ? cartWithDraft.metadata.checkout_draft
      : null;
  ...
  events.forEach(function (ev) {
    ...
    if (ev.event_type === 'add_to_cart') {
      ...
      cartKit = kitFromEventMetadata(ev.metadata) || cartKit;
    }
  ...
  const journeySteps = [
    ...
    { id: 'details', label: 'Details filled', at: null },
  ...
  if (checkoutDraft && checkoutDraft.updated_at && !journeySteps[5].at) {
    journeySteps[5].at = checkoutDraft.updated_at;
  }
  if (order && !journeySteps[7].at) journeySteps[7].at = order.created_at;
  ...
  return {
    ...
    customer: {
      name:
        ...
        (checkoutDraft && checkoutDraft.name) ||
        (lead && lead.name) ||
        null,
      email:
        (order && order.customer_email) ||
        (checkoutDraft && checkoutDraft.email) ||
        (lead && lead.email) ||
        null,
      phone:
        (order && order.customer_phone) ||
        (checkoutDraft && checkoutDraft.phone) ||
        null,
      country:
        (order && order.country) ||
        (checkoutDraft && checkoutDraft.country) ||
        latestSession.country ||
        visitor.country ||
        null,
      city: (checkoutDraft && checkoutDraft.city) || null,
      address: (checkoutDraft && checkoutDraft.address) || null,
      apartment: (checkoutDraft && checkoutDraft.apartment) || null,
      state: (checkoutDraft && checkoutDraft.state) || null,
      postcode: (checkoutDraft && checkoutDraft.postcode) || null,
      checkout_stage: (checkoutDraft && checkoutDraft.stage) || null,
    ...
    product: {
      current: composeProductDisplay(
        cartProduct ||
          (Object.keys(viewedMap)[0]
            ? viewedMap[
                Object.keys(viewedMap).sort(function (a, b) {
