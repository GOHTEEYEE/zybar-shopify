...
function buildVisitorMap(sessions, events, orders) {
  ...
  (sessions || []).forEach(function (session) {
    ...
    // Use session start as arrival; keep last activity for stay length.
    touchTimestamp(row, session.started_at);
  ...
  return Object.keys(map).map(function (visitorId) {
    ...
    row.duration_seconds = calcDurationSeconds(row.first_seen_at, row.last_active_at);
    return row;
  });
}

function calcDurationSeconds(firstSeenAt, lastActiveAt) {
  if (!firstSeenAt || !lastActiveAt) return 0;
