/**
 * Permanent member-pricing identity and tier service.
 *
 * Browser state is represented by a signed, expiring credential. The server
 * remains authoritative: a valid credential must map to an active subscriber.
 */
...
function getTierForSubscriber(subscriber, preferredTier) {
...
function issueCredential(subscriber, tierId, env) {
  ...
  const tier = getTierForSubscriber(subscriber, tierId);
  const payload = {
    v: TOKEN_VERSION,
...
async function findActiveSubscriber(supabase, options, env) {
  ...
  let query = supabase
    ...
    .select('id, status, discount_code, visitor_id, session_id');
  ...
  return {
    ...
    tier: getTierForSubscriber(result.data, verified && verified.tier)
  };
}
