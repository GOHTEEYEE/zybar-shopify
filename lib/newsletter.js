...
async function subscribeNewsletter(options, env) {
  ...
  // Prefer brand-scoped lookup when column exists.
  let existing = null;
  let lookupError = null;
  const brandedLookup = await supabase
    .from('newsletter_subscribers')
    .select('id, email, discount_code, status, source, brand')
    .ilike('email', email)
    .eq('brand', BrandAnalytics.BRAND_ZYBAR)
    .maybeSingle();
  if (!brandedLookup.error) {
    existing = brandedLookup.data;
  } else {
    ...
    if (existing && BrandAnalytics.isLunevaLead(existing)) {
      // Same email is LUNEVA-only — do not hijack; create ZYBAR row if brand column allows.
      existing = null;
    }
  }
  ...
  if (existing) {
    if (BrandAnalytics.isLunevaLead(existing)) {
      // Should not reach here when brand column works; avoid clobbering LUNEVA.
      existing = null;
    }
  }
  ...
  if (insertError && /brand|column/i.test(String(insertError.message || ''))) {
    ...
    const retryBrand = await supabase
      .from('newsletter_subscribers')
      .insert(insertPayload)
      .select('id, email, discount_code, created_at')
      .single();
    inserted = retryBrand.data;
    insertError = retryBrand.error;
  }
