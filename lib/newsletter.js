...
async function subscribeNewsletter(options, env) {
  ...
  try {
    ...
    scheduledExecutions = await WorkflowEngine.scheduleLeadSignupWorkflows(
      supabase,
      Object.assign({}, row, {
    ...
    console.warn('newsletter schedule workflow:', workflowErr && workflowErr.message ? workflowErr.message : workflowErr);
  }
  ...
  return {
    ...
    json: {
      ...
      workflowScheduled: scheduledExecutions.length > 0,
