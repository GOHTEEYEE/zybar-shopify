...
async function deleteJourneyPermanently(supabase, journeyId) {
  ...
  // Cancel any still-pending queue rows first (history stays in marketing_history if separate).
  await supabase
    .from('action_queue')
    .update({
      status: 'cancelled',
      error_message: 'Cancelled because journey was permanently deleted',
      updated_at: nowIso()
    })
    .eq('journey_id', journeyId)
    .eq('status', 'pending');
