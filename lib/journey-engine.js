...
async function getJourneysByTrigger(supabase, triggerType, brand) {
  ...
  let query = supabase
    ...
    .eq('status', 'published');
  // Prefer brand-scoped journeys when the column exists; fall back to all published.
  const branded = await query.eq('brand', brandKey);
  if (!branded.error) {
    return branded.data || [];
  }
  if (!/brand|column/i.test(String(branded.error.message || ''))) {
    throw branded.error;
  }
  const result = await supabase
    .from('journeys')
  ...
  if (brandKey === BrandAnalytics.BRAND_LUNEVA) {
    return rows.filter(function (j) {
      return String(j.journey_key || '').indexOf('luneva_') === 0;
    });
  }
  return rows.filter(function (j) {
    return String(j.journey_key || '').indexOf('luneva_') !== 0;
...
function buildTemplateVariables(lead, snapshot, env) {
  ...
  const isLuneva = BrandAnalytics.isLunevaLead(lead);
  const tier = MemberPricing.getTierForSubscriber(lead);
  return {
    customerName: firstOrder && firstOrder.customer_name ? firstOrder.customer_name : null,
    discountCode:
      (lead && lead.discount_code) ||
      (isLuneva ? EmailTemplates.LUNEVA_DISCOUNT_CODE : EmailTemplates.DEFAULT_DISCOUNT_CODE),
    memberCredential: MemberPricing.issueCredential(lead, tier.id, env),
    storeName: isLuneva
      ? EmailTemplates.LUNEVA_STORE_NAME
      : env.STORE_NAME || EmailTemplates.DEFAULT_STORE_NAME,
...
/**
 * Transition a lead into the single published journey for a trigger type + brand.
...
async function enrollLeadByTrigger(supabase, lead, triggerType) {
  ...
  const BrandAnalytics = require('./brand-analytics.js');
  const brand = BrandAnalytics.inferLeadBrand(lead);
  const journeys = await getJourneysByTrigger(supabase, triggerType, brand);
  if (!journeys.length) return [];
  if (journeys.length > 1) {
    throw new Error(
      'Multiple active journeys found for trigger: ' + triggerType + ' brand: ' + brand
    );
  }
...
async function enrollLeadOnNoPurchase(supabase, lead) {
  const BrandAnalytics = require('./brand-analytics.js');
  // Win-back is ZYBAR-only for now.
...
async function transitionNoPurchaseLeads(supabase, options) {
  ...
  const customerJourneys = await getJourneysByTrigger(supabase, 'purchase', 'zybar');
...
async function createJourney(supabase, payload) {
  ...
  const insert = await supabase
    ...
    .insert({
      ...
      brand:
        payload.brand === 'luneva' || String(journeyKey).indexOf('luneva_') === 0
          ? 'luneva'
          : 'zybar',
    ...
    .select('*')
    .single();
  if (insert.error && /brand|column/i.test(String(insert.error.message || ''))) {
...
const PROTECTED_JOURNEY_KEYS = {
  ...
  luneva_welcome_journey: true,
  luneva_cart_journey: true,
  luneva_customer_journey: true
};
