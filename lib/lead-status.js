...
async function listActiveLeads(supabase, options) {
  ...
  // Campaigns/journeys must never see test leads; the CRM can opt in.
  ...
  if (result.error) {
    ...
    if (/status|column/i.test(String(result.error.message || ''))) {
      ...
      return BrandAnalytics.filterZybarLeads(fallback.data || []);
    }
    throw result.error;
  }
  return BrandAnalytics.filterZybarLeads(result.data || []);
}
