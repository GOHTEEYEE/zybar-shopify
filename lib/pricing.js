...
function mergeProductsJsonFallback(catalog) {
  ...
  var perProductCompare = (json && json.perProductCompareAtPricesBySize) || {};
  ...
  (json.products || []).forEach(function (product) {
    ...
    if (!existing) {
      catalog.products[slug] = {
        ...
        customDesignFeeUsd: product.customDesignFeeUsd,
        prices: fallbackPrices,
        compareAtPrices: fallbackCompare
      };
      return;
    }
    ...
    if (product.customDesignFeeUsd != null) existing.customDesignFeeUsd = product.customDesignFeeUsd;
    // perProductPricesBySize is an intentional override (e.g. sale tiers) — always win.
    if (perProduct[slug]) {
      existing.prices['30x45'] = fallbackPrices['30x45'];
      existing.prices['40x60'] = fallbackPrices['40x60'];
    } else {
    ...
    if (perProductCompare[slug]) {
      existing.compareAtPrices['30x45'] = fallbackCompare['30x45'];
      existing.compareAtPrices['40x60'] = fallbackCompare['40x60'];
    } else {
      if (!Number(existing.compareAtPrices['30x45']) && fallbackCompare['30x45']) {
        existing.compareAtPrices['30x45'] = fallbackCompare['30x45'];
      }
      if (!Number(existing.compareAtPrices['40x60']) && fallbackCompare['40x60']) {
        existing.compareAtPrices['40x60'] = fallbackCompare['40x60'];
      }
    }
  });
  ...
  // LUNEVA email welcome — keep catalog aligned with member-pricing tier (15%).
