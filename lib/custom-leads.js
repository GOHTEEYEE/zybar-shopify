...
/** Fast exact count for dashboard KPIs — skips enrichRows N+1 lookups. */
async function count(supabase, query) {
  if (!supabase) throw new Error('Database not configured');
  query = query || {};
  var range = CustomerActivity.parseRange
    ? CustomerActivity.parseRange(query)
    : { start: null, end: null };
