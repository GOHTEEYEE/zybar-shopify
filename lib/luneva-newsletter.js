 ...
 * Enrolls brand-scoped LUNEVA welcome journey on new signup.
 */
...
async function subscribeLunevaNewsletter(options, env) {
  ...
  try {
    enrolledJourneys = await JourneyEngine.enrollLeadOnSignup(supabase, inserted);
  } catch (enrollErr) {
    console.warn(
      'luneva welcome enroll:',
      enrollErr && enrollErr.message ? enrollErr.message : enrollErr
    );
  }

  const response = memberResponse(inserted, false);
  if (enrolledJourneys && enrolledJourneys.length) {
    response.json.enrolled_journeys = enrolledJourneys.length;
  }
  return response;
}
