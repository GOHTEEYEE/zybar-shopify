...
async function getLeadJourneySnapshot(supabase, lead) {
  ...
  if (flags.hasPurchase) status = 'customer';
  else if (flags.hasCheckout || (latestCart && latestCart.status === 'checkout_started')) status = 'checkout';
  else if (flags.hasAddToCart || (latestCart && (latestCart.item_count || 0) > 0)) status = 'cart';
  else if (flags.hasProductView) status = 'browsing';
  ...
  return {
    ...
    last_activity_type:
      ...
      (orders[0] ? 'purchase' : 'email_signup'),
